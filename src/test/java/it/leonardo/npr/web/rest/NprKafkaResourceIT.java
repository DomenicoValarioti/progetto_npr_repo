...
import it.leonardo.npr.config.KafkaProperties;
...
class NprKafkaResourceIT {
    ...
    void setup() {
        ...
        NprKafkaResource kafkaResource = new NprKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/npr-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/npr-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
